...
const categoryImages = {
  ...
  Electrician: '/images/electricisan.jpg',
...
const BrowseTasks = () => {
  ...
  const [bookmarkedTasks, setBookmarkedTasks] = useState([]); // To track bookmarked tasks
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      try {
        ...
        // Initialize bookmarkedTasks state (all unbookmarked by default)
        setBookmarkedTasks(data.map((task) => ({ taskId: task._id, isBookmarked: false })));
      } catch (error) {
            ...
            return (
              ...
              >
                <figure className="px-4 pt-4">
                  <img
                    src={categoryImages[task.category] || '/images/default.jpg'} // Default image if not found
                    alt={task.category}
